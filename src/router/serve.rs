...
use tuwunel_core::{Result, debug_info, info};
...
pub(super) async fn serve(services: Arc<Services>, handle: ServerHandle) -> Result {
	...
	let tcp_log_addrs = addrs.iter().map(|addr| format!("tcp:{addr}"));

	let unix_log_addr = socket_path.as_ref().map(|socket_path| {
	...
	let passed_unix_log_addrs = passed_unix_listeners.iter().map(|listener| {
		let addr = listener.local_addr()?;
		Ok(format!("passed:unix:{addr:?}"))
	});

	let log_addrs = tcp_log_addrs
		.map(Ok)
